			...
			<div id="primary-two" class="widget-area">
				<?php
                    $output = '<ul class="recentposts">';
                        $output .= '<li><h3>' . __('Recent Posts', 'flat-material') . '</h3></li>';
                        ...
                        if ( $recentposts_query->have_posts() ) : while ( $recentposts_query->have_posts() ) : $recentposts_query->the_post();
                            $output .= '<li>';
                                ...
                                $month_check = $month;
                            $output .= '<h4><a href="' . get_the_permalink() . '" title="' . sprintf( __('Permalink to %s', 'flat-material'), the_title_attribute('echo=0') ) . '" rel="bookmark">' . get_the_title() . '</a></h4>';
                            $output .= '</li>';
				...
				<br />
				<ul>
					<li><h3 class="border-bottom"><?php _e('Categories', 'flat-material'); ?></h3></li>
					<?php
					...
					<?php if ( !is_author() ) : ?>
						<li>&nbsp;</li>
						<li><a href="<?php echo home_url( '/blog/' ); ?>" class="mdl-button mdl-js-button mdl-button--raised"><?php _e('more', 'flat-material'); ?></a></li>
					<?php endif; ?>
				</ul>
			</div><!-- /#primary-two -->
		</div>
